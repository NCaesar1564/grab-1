...
const NavL = () => {
  ...
  useEffect(() => {
    axios.get('/data/data.gallery.json').then(d => setGallery(d.data)).catch(err => console.error(err))
  }, [])
  return (
    <div className='flex justify-center items-center'>
      <div className='flex justify-center items-start gap-x-20 not-md:gap-x-5'>
        {/* div1 */}
        <div className='flex flex-col items-start gap-y-5  not-md:hidden'>
        ...
        {/* div2 */}
        <div className='flex flex-col items-start gap-y-3 not-md:gap-2'>
        ...
        <div className='flex flex-col items-start gap-y-3 not-md:gap-2'>
          ...
          {Link.map((d) => {
            return (
              <a key={d.id} href={d.href} className='hover:text-[#195A00] not-md:underline underline-offset-2'>{d.name}</a>
            )
          })}
        </div>
        {/* div4 */}
        <div className='w-[336px]  flex flex-col items-start gap-y-3 pb-3 not-md:hidden'>
